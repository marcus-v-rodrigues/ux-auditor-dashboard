...
O Dashboard normaliza a sessão a partir das claims do Janus e expõe um shape único para autenticação e RBAC. A prioridade de origem é:

1. `profile.roles` vindo do `userinfo`/`profile` do Janus
2. claims do `id_token`
3. claims do `access_token` como fallback defensivo
4. roles anteriores da sessão apenas para preservar a autorização durante refresh
...
type Session = {
  user: {
    ...
    roles: {
      global: string[];
      client: Array<{ code: string; clientId: string }>;
    };
  };
  roles: {
    global: string[];
...
O app usa essa sessão já normalizada:
...
export default async function Page() {
  ...
  // Claims disponíveis:
  // - session.accessToken: Token JWT completo para chamadas server-side
  // - session.user: Objeto consistente com dados do usuário
  // - session.user.id: ID único do usuário
  // - session.user.roles: RBAC estruturado { global, client }
  // - session.scopes: Lista de escopos autorizados
  // - session.roles: RBAC estruturado { global, client }
  // - session.claims: Claims consolidadas do token

  return <div>Usuário: {session.user.id}</div>;
}
```
...
O proxy usa a sessão normalizada e decide acesso com esta regra:

```typescript
const roles = session.roles ?? session.user.roles;

const canAccessUxAuditor =
  roles.global.includes("janus_admin") ||
  roles.client.some((role) => role.clientId === "ux-auditor");
```

O resultado é diferenciado em três casos:

1. Não autenticado: redireciona para `/auth/signin`
2. Autenticado sem roles válidas: redireciona para `/auth/error?error=AccessDeniedNoRoles`
3. Autenticado sem acesso ao `clientId = ux-auditor`: redireciona para `/auth/error?error=AccessDeniedClient`
...
- Callback JWT para gerenciamento do ciclo de vida dos tokens e da sessão normalizada
- Callback de sessão para expor `session.roles` e `session.user.roles` de forma consistente
...
- Redireciona usuários sem roles válidas para `AccessDeniedNoRoles`
- Redireciona usuários sem acesso ao `clientId = ux-auditor` para `AccessDeniedClient`
...
- `session.roles` e `session.user.roles` são a mesma estrutura RBAC normalizada
...
- O dashboard trata o provider Janus como `oidc`
- As roles são preferencialmente lidas de `profile.roles`/`userinfo`, depois de `id_token` e por último do `access_token`
- O callback não depende do `access_token` como fonte principal de RBAC
...
3. Novos tokens são recebidos e a sessão é reidratada
4. As roles anteriores são preservadas se o refresh não trouxer claims suficientes
5. Se a renovação falhar, um erro é marcado na sessão
...
- `AccessDeniedClient`: Sem acesso ao `clientId = ux-auditor`
- `AccessDeniedNoRoles`: Sessão autenticada sem roles válidas
